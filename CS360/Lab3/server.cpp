...
//  Lab3Multithreading
//
//  Created by Keith Calderwood on 2/1/16.
//  Copyright (c) 2016 Keith Calderwood. All rights reserved.
//
...
public:
    void push(int sock) {
        ...
        sem_wait(&mutexx);
        stlqueue.push(sock);
        sem_post(&mutexx); //increments and wakes up the waiting processes
        sem_post(&full);
    }
    int pop() {
        sem_wait(&full);
...
void writeImage(int skt, std::string rsrc) {
    ...
    while (write_amt < fileLen) {
        int new_write = write(skt, buffer, fileLen);
        if(new_write == -1)
            std::cout << "Write failed1" << std::endl;
        write_amt += new_write;
    }
}
...
void writeText(int skt, std::string rsrc) {
    ...
    if (writeBytes == -1)
        std::cout << "Write failed2" << std::endl;
}
...
void send404(int skt) {
    ...
    if (writeBytes == -1)
        std::cout <<"Write failed3" << std::endl;
    std::cout << "Response: \n" << msg << std::endl;
}

void makeDirHTML(int skt, std::string dir, std::vector<std::string> files) {
    ...
    if(msgBytes == -1)
        std::cout << "Write failed4" << std::endl;
    int bodyBytes = write(skt, dirHTML.c_str(), dirHTML.size());
    if(bodyBytes == -1)
        std::cout <<"Write failed5" << std::endl;
    std::cout << "Response: \n" << msg << std::endl;
}
...
void serve(int skt) {
    ...
    if(S_ISREG(filestat.st_mode)) {
        ...
        int writeBytes = write(skt, msg.c_str(), msg.size());
        if(writeBytes == -1)
            std::cout << "Write failed6" << std::endl;
        std::cout << "Response: \n" << msg << std::endl;
    ...
    if(S_ISDIR(filestat.st_mode)) {
        ...
        if(foundIndex) {
            ...
            int writeBytes = write(skt, msg.c_str(), msg.size());
            if (writeBytes == -1)
                std::cout << "Write Failed7" << std::endl;
...
void *serveQueue(void *arg) {
    for(;;) {
        int sock = sockqueue.pop();
        serve(sock);
        int closed = close(sock);
        if (closed != 0)
            std::cout << "Socket didn't close :(" << std::endl;
    }
}
...
int main(int argc, char* argv[]) {
    ...
    sem_init(&empty, PTHREAD_PROCESS_PRIVATE, threadno);
    sem_init(&mutexx, PTHREAD_PROCESS_PRIVATE, 1);
    ...
    for(;;)
    {
        ...
        sockqueue.push(conn_sock);
        
        pthread_create(&thread, NULL, serveQueue, (void*) 0);
    }
    pthread_exit(NULL);
...
for ((i=0; i<20; i++))
do
GET -dUe http://ec2-54-218-67-46.us-west-2.compute.amazonaws.com:3005/test1.html &
done*/
